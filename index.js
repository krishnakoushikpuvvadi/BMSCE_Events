...
app.get("/Protocolhome", (req, res) => {
  console.log("get data is : ",req.cookies.isAdmin)
  const isAdmin = req.cookies.isAdmin === "no";
  console.log("Is admin is : ",isAdmin)
  res.render("ProtocolHome", { isAdmin: "puvvadikk.cs21@bmsce.ac.in" });
});
...
app.get("/eventdetails", (req, res) => {
  ...
  db.query(eventdetailsQuery, [name], (err, results) => {
    if (err) throw err;
    res.render("EventDetails.ejs", { eventData: results });
  });
});
...
app.post("/userLogin", (req, res) => {
  ...
  db.query(selectUser, [mail], (err, results) => {
    ...
    const isAdmin = results[0] && results[0].isAdmin === 1 ? "yes" : "no";
...
app.post("/Registration", (req, res) => {
  ...
  db.query(checkEmail, [email], (err, results) => {
    ...
    if (results.length > 0) res.render("registrationpage", { msg: "Email exists already", std: results });
    else {
...
app.post("/joinEvent", (req, res) => {
  ...
  db.query(selectEventId, [eventName], (selectEventErr, selectEventResult) => {
    ...
    db.query(checkRegistered, [email, selectEventResult[0].event_join_id], (err, checkResults) => {
      if (err) throw err;
